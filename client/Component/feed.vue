    ...
    <template v-if="isloaded">
      <div v-for="(post, i) in allPosts" :key="i" class="_card _mar_b20">
          ...
          <div class="_card_top_more">
            <Dropdown trigger="click" placement="bottom-end">
              ...
              <DropdownMenu slot="list">
                ...
                <DropdownItem
                  ><p
                    @click="postDelete(post, i)"
                    class="_drop_text _drop_pre_icon"
                  >
        ...
        <div class="_card_body">
          ...
          <div
            v-for="(postImages, i) in postWisePictures[post.id]"
            :key="i"
            class="_card_status_pic_all"
          >
            <div @click="isModal = true" class="_card_status_pic">
              <img
                ...
                v-bind:src="
                  'http://localhost:3333/uploads/' + postImages.post_image
                "
                lazy="loaded"
              />
            </div>
          ...
          <div class="_1card_comment_box_input_icon">
            <div class="_1card_comment_box_input">
              <input
                ...
                v-model="commentObj.Comment_Text[post.id]"
                type="text"
              ...
              <div class="_comment_more">
                <Dropdown trigger="click" placement="bottom-end">
                  ...
                  <DropdownMenu slot="list">
                    ...
                    <DropdownItem
                      ><p @click="commentDelete(comment, i)" class="_drop_text">
                        Delete
                      </p></DropdownItem
                    >
                  </DropdownMenu>
                    ...
                    <div class="_comment_more">
                      <Dropdown trigger="click" placement="bottom-end">
                        ...
                        <DropdownMenu slot="list">
                          ...
                          <DropdownItem>
                            <p @click="ReplycommentDelete(reply, i)">Delete</p>
                          </DropdownItem>
                        </DropdownMenu>
                      </Dropdown>
                    </div>
                ...
                <div class="_1card_comment_box_input_icon">
                  <div class="_1card_comment_box_input">
                    <input
                      ...
                      v-model="commentReplyObj.Reply_Text[comment.id]"
                      type="text"
      ...
      <div class="_card _mar_b20">
        ...
        <div class="_comment_main">
          ...
          <div class="_comment_details">
            ...
            <div class="_comment_status">
              <p class="_comment_status_text">new comment</p>
            </div>
            <div class="_comment_reply">
              <div class="_comment_reply_num">
                <ul class="_comment_reply_list">
            ...
            <div class="_reply" keys="rep0">
              <div class="_reply_main">
                ...
                <div class="_comment_details">
                  ...
                  <div class="_comment_status">
                    <p class="_comment_status_text">sdfsdf</p>
                  </div>
                  <div class="_comment_reply">
                    <div class="_comment_reply_num">
                      <ul class="_comment_reply_list">
...
export default {
  computed: {
    ...mapGetters({
     ...
     allreplycomment: "getallreplycomment",
  ...
  methods: {
    ...
    async postDelete(postObj, index) {
      console.log(postObj);
   //   this.postWiseComments.splice(index)
      const res = await this.callApi(
        "post",
    ...
    async commentDelete(comment, index) {

     // this.postWiseComments.splice(index,1)
      
      const res = await this.callApi(
        "post",
        "http://localhost:3333/commentdelete/" + comment.id
      );
      console.log(res)
      if (res.status == 200) {
        console.log("remove comment");
        this.$store.commit("removecomment", index);
         this.postWiseComments[comment.id]--
        console.log(res);
      }
      
     
    },
    async ReplycommentDelete(reply, index) {

    //this.commentWiseReply.splice(index,1)
   
      const res = await this.callApi(
        "post",
        "http://localhost:3333/commentReplydelete/" + reply.id
    );
      console.log(res)
      if (res.status == 200) {
        console.log("reply_comment");
        this.$store.commit("removereplyComment", index);
        console.log(res);
      }
    },
    async addComment(post) {
      // console.log(post);
      this.commentObj.post_id = post.id;
      this.commentObj.user_id = this.CurrentUserInfo.id;
      this.commentObj.comment_text = this.commentObj.Comment_Text[post.id]

      const res = await this.$axios.$post("comment", this.commentObj);

      // console.log(res)

      
      this.postWiseComments[post.id].push(res);
      this.$store.commit("setAllcomments",this.postWiseComments[post.id])
      
      this.commentObj.Comment_Text.splice(post.id,1)
      this.commentCount[post.id]++;
      this.commentObj.comment_text = ""
      //console.log(this.commentCount[post.id]);
    },
    async addCommentReply(comment) {
      //  console.log(comment)
      this.commentReplyObj.comment_id = comment.id;
      this.commentReplyObj.user_id = this.CurrentUserInfo.id;
      this.commentReplyObj.reply_text = this.commentReplyObj.Reply_Text[comment.id]

      const res = await this.$axios.$post("commentReply", this.commentReplyObj);

     this.commentWiseReply[comment.id].push(res);
     this.$store.commit("setAllreplycomment",this.commentWiseReply[comment.id])
     
     this.commentReplyObj.Reply_Text.splice(comment.id,1)
      ...
      .then((response) => {
       ...
       // console.log(response.data);
        response.data.forEach((post) => {
          // this.$store.commit('setAllcomments', post.comments)
          this.likeCount[post.id] = post.like.length;
          // this.test = post.like.length
          this.commentCount[post.id] = post.comments.length;
          this.postWiseComments[post.id] = post.comments;
        
          this.postWisePictures[post.id] = post.postImage;
          post.comments.forEach((comment) => {
            this.$store.commit("setAllcomments", post.comments)
            this.commentWiseReply[comment.id] = comment.commentReply;
            this.$store.commit("setAllreplycomment",comment.commentReply)
          });
          //  this.commentWiseReply[comment.id] = comment.commentReply
        });
      })
